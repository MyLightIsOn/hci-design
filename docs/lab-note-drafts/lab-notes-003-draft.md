...
*Most design systems treat accessibility as a property of visual elements: contrast ratios, type scales, labeled components. That is not a foundation. That is a surface. This article argues that accessible design requires a different kind of system entirely, one that encodes interaction behavior as its starting point, and examines what the research says about whether AI can use such a system, or whether it is too early to ask.*
...
This is not a developer awareness problem. Accessibility has never had more documentation, more tooling, more conference talks, or more organizational commitment than it does now. Mack et al. (2021) surveyed every accessibility paper published at CHI and ASSETS from 1994 to 2019 and found the field grew faster than CHI itself, reaching nearly 8% of all CHI papers by 2019. The research output is substantial. The web is not getting more accessible.

Paiva et al. (2021) reviewed 94 studies on accessibility in software engineering practice and found that the dominant research focus across three decades was testing and design, with testing concentrated at the quality assurance phase. Accessibility audits happen at the end of the pipeline. Solutions are applied in code. The best fix for many complex accessibility issues, the authors note, would be to revisit the overall approach rather than patch the output. Almost no teams do that, because the process does not ask them to.

When AI entered the picture, the pattern held. Aljedaani et al. (2024) evaluated 88 websites generated by ChatGPT and found that 84% had accessibility problems. The types of violations were not novel. They mirrored the issues found in websites built by human developers. The model did not introduce new failure modes. It replicated existing ones at speed. If you want to understand why accessibility keeps failing at the same places every year, that replication pattern is the answer. The errors are downstream symptoms of something built into the infrastructure above them.
...
The argument that accessibility belongs at the behavior and interaction layer is not recent. In 2001, Stephanidis et al. described what a unified user interface would require at an architectural level: a repository of alternative implemented dialogue artifacts, user and context information, decision-making capability that selects among alternatives given that information, and interface control to apply the decisions made. They identified the unsolved problem as the representation and deployment of design logic in computable forms, to enable what they called run-time design assembly.
