...
The designers who take this seriously have concrete options, and most of them are old. Bainbridge's remedies were design remedies. She wanted operators scheduled onto hands-on control, drilled in simulators for the rare failures, and given automation that fails visibly instead of gracefully. Her modern translations are recognizable. Let the human attempt the hard call before the AI shows its answer, the pattern the clinical literature would have wanted in the colonoscopy suite. Surface the model's confidence unevenly, so scrutiny is invited exactly where the system is weak, the calibration argument from [Lab Notes 001](/lab-notes/trust-as-resource) applied to skill instead of trust. And build unassisted reps into the workflow and instrument them, because the Polish study could only detect deskilling because randomization preserved an unassisted baseline. Most products have no such baseline. Their users' expertise could be draining and no one, including the users, would have the data to notice.
...
<figure>
  <img src="/lab-notes-images/004/3.png" alt="Circular diagram titled 'The human-in-the-loop architecture' showing four stages: AI system analyzes data and generates output, human review where an expert evaluates the AI output, decision/action where accepted output informs decisions, and feedback/data where outcomes flow back to improve the system. At the center, a cracked column labeled 'fragile expertise' notes that unassisted skills erode without regular use, and judgment atrophy weakens the loop. A callout reads: the loop depends on a foundation, and expert judgment only stays sharp if the loop preserves unassisted practice. A warning box states that if the loop does not preserve opportunities for unassisted practice, expertise erodes, human review weakens, and the entire system becomes fragile." />
  <figcaption>The human-in-the-loop architecture assumes the human stays expert. Nothing in the loop itself preserves that assumption.</figcaption>
</figure>
...
None of this argues against the architecture, only that it carries a maintenance cost nobody is budgeting. If a system consumes expert judgment as its backstop, it should be designed to keep that judgment exercised, the way aviation eventually accepted that automated cockpits require deliberate manual practice. Expertise, like trust, is a resource the system spends. It should be stewarded like one.
